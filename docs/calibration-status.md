...
- `data/keroppi-copy-paper.json` の 2026-09-12 実測を反映した記録を確認した。
- 7本の描画ストロークはすべて円弧で、直線 draw の実測ではない。geometry（中心、半径、開始角、掃引角）はサンプル側と一致している。
- 実測では外周弧を左右 `10 / 14` または `14 / 10`、目・口の小弧を左右 `20 / -1` などに個別設定し、描画時間も `3050〜410ms` に個別調整している。travel と turn にも個別の時間・距離倍率がある。
- このため、実測値から `deadArcMmPerSecAtDrawSpeed` の単一の共通値を推定することはできない。円弧の速度比・小半径での挙動・turn/travel の影響が混在しており、精度もまだ悪い。
- けろけろけろっぴ専用の初期値をサンプルJSONへ追加しても、現在のサンプル読込は `config` をアプリ全体の設定へ取り込み保存するため、他サンプルを読み込む前の共通状態を変えてしまう。現実測では共通初期値を変更せず、個別補正の記録として扱う。
- 描画順最適化は `planningOptions` のopt-in機能として実装した。けろけろけろっぴではturn回数を第一優先、同数ならcube間のtravel距離を第二優先にしてストローク順を選ぶ。
- `turn-to-travel` はtravel方向との差が30度以内の場合だけ省略し、省略時は現在の向きのまま直進したcube到達位置をシミュレーションへ反映する。`turn-to-draw` は省略しない。
- `commandOverrides` を持つJSONでは、seg IDが描画順に依存するため、この最適化を無効化する。
